...
def make_arrow(root, dataset_root):
    ...
    with open(f"{root}/OpenEnded_mscoco_test2015_questions.json", "r") as fp:
        questions_test2015 = json.load(fp)["questions"]
    with open(f"{root}/OpenEnded_mscoco_test-dev2015_questions.json", "r") as fp:
    ...
    for split in [
        ...
        with pa.OSFile(f"{dataset_root}/vqav1_{split}.arrow", "wb") as sink:
            with pa.RecordBatchFileWriter(sink, table.schema) as writer:
                writer.write_table(table)

    table = pa.ipc.RecordBatchFileReader(
        pa.memory_map(f"{dataset_root}/vqav1_val.arrow", "r")
    ).read_all()
    ...
    with pa.OSFile(f"{dataset_root}/vqav1_trainable_val.arrow", "wb") as sink:
        with pa.RecordBatchFileWriter(sink, df1.schema) as writer:
            writer.write_table(df1)

    with pa.OSFile(f"{dataset_root}/vqav1_rest_val.arrow", "wb") as sink:
