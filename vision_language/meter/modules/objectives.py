...
def compute_snli(pl_module, batch):
    ...
    clip_mask = (snli_labels==0).to(torch.float)
    infer = pl_module.infer(
        batch, mask_text=False, mask_image=False, clip_mask=clip_mask
    )
...
def compute_irtr(pl_module, batch):
    ...
    clip_mask = torch.zeros((_bs, false_len + 1), device=images.device)
    clip_mask[:, 0] = 1
    ...
    infer = pl_module.infer(
        ...
        clip_mask=rearrange(clip_mask, "bs fs -> (bs fs)")
    )
