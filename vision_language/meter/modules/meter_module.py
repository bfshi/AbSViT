...
from .vit import vit_tiny_patch16_224, vit_small_patch16_224, vit_base_patch16_224
from .absvit import absvit_tiny_patch16_224, absvit_small_patch16_224, absvit_base_patch16_224
...
class METERTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        self.is_swin = config['is_swin']
        self.is_vit = config['is_vit']
        ...
        if torch.distributed.is_initialized():
            if torch.distributed.get_rank() == 0:
                if self.is_clip:
                    ...
                    eval(config['vit'] + f'(path=\'{vit_path}\')')
        ...
        if self.is_clip:
            ...
            text_dim = config['input_text_embed_size']
            self.vit_model = eval(config['vit'] + f'(path=\'{vit_path}\', text_dim={text_dim})')
        ...
        # ===================== freeze vit and text backbone =====================

        if config["freeze_backbone"]:
            for k, p in self.vit_model.named_parameters():
                if 'decoder' not in k and 'prompt' not in k and 'top_down_transform' not in k and 'head' not in k:
                    p.requires_grad = False
            for k, p in self.text_transformer.named_parameters():
                p.requires_grad = False
    ...
    def infer(
        ...
        if self.hparams.config['feedback']:
            image_embeds, var_loss = self.vit_model(img, return_var_loss=True, prompt_=prompt, clip_mask=clip_mask)
        else:
        ...
        if self.is_clip or self.is_vit:
        ...
        if self.hparams.config['feedback']:
            ret['var_loss'] = self.hparams.config['var_loss'] * var_loss
